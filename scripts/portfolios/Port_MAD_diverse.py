...
#=============================================================================
# Define mMAD measure parameters alpha and coef
L = 3
coef = np.full(L, 1/L)
...
rtype = "MaxDiverse"
port4 = p4.set_model(coef=coef, rtype=rtype)   

ww = p4.get_weights()
p4.port_view(title="mMAD Max Diversified Portfolio", ylabel="price ($)")
p4.port_view_all()
p4.port_perf()
...
port = []
for rty in rtype:
    port4 = p4.set_model(coef=coef, rtype=rty)  
    port4.columns = [rty]
    port.append(port4)
...
pp.port_view_all(componly=True, 
                 title="mMAD Portfolios - Relative Performance", 
                 xlabel="year")
